...
export const saveNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveNegotiationTabColmedica(req?.body);
...
export const saveNegotiationTabFareBaseColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveNegotiationTabFareBaseColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const saveNegotiationTabServiceColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveNegotiationTabServiceColmedicaColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const saveNegotiationTabTypeIncrementColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveNegotiationTabTypeIncrementColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const saveNegotiationTabRendomColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveNegotiationTabRendomColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const saveNegotiationTabPlansColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveNegotiationTabPlansColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const saveLogicNegotiationTabCupsColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.saveLogicNegotiationTabCupsColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const updateNegotiationTabServiceColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.updateNegotiationTabServiceColmedica(req?.body);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const updateNegotiationTabPlansColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, data } = await repository.updateNegotiationTabPlansColmedica(
			id,
			updates
		);
...
export const updateNegotiationTabRendomColmedica: RequestHandler = async (req, res) => {
	try {
		...
		if (
			...
			!Array.isArray(idTypeRendom) ||
			!Array.isArray(idTypeService)
		) {
			return res.status(400).json({
				message:
					'Invalid request body. Ensure id_NegotiationTabColmedica is provided and idTypeRendom, idTypeService are arrays.'
			});
		}
...
export const updateNegotiationTabTypeIncrementColmedica: RequestHandler = async (req, res) => {
	try {
		...
		if (
			!id_NegotiationTabColmedica ||
			!Array.isArray(idTypeIncrement) ||
			!Array.isArray(valueIncrement) ||
			idTypeIncrement.length !== valueIncrement.length
		) {
			return res.status(400).json({
				message: 'Invalid request body'
			});
...
export const getProvidersColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getProvidersColmedica(idProvider);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getNegotiationDetails: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getNegotiationDetails(numericId);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getInfoOfficeProvider: RequestHandler = async (req, res) => {
	try {
		const idProvider = req.query.idProvider;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getInfoOfficeProvider(idProvider);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getContactsProviderColmedica: RequestHandler = async (req, res) => {
	try {
		const idProvider = req.query.idProvider;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getContactsProviderColmedica(idProvider);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getOfficeProvidersColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getOfficeProvidersColmedica(idOfficeProvider);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getProductColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getProductColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getTypeServiceColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeServiceColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getReferenceRateColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getReferenceRateColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getOccupationColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getOccupationColmedica(idOccupation);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getServicesColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getServicesColmedica(idClasificationTypeService);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getInfoLogicColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getInfoLogicColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getInfoLogicTableColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getInfoLogicTableColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getPlansColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getPlansColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getTypeFaresColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeFaresColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getTypeRendomColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeRendomColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getTypeIncrementColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeIncrementColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const loadFileNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, data }: IResponse<IResponseCreate> =
			await repository.postLoadFileNegotiationColmedica(fileData);
...
export const getClasificationTypeServiceColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getClasificationTypeServiceColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getNegotiationTabColmedica();
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const deleteNegotiationTabServiceColmedicaController: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, data } = await repository.deleteNegotiationTabServiceColmedica(
			Number(idNegotiationTabServiceColmedica)
		);
...
export const getGroupByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getGroupByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getSpecialityByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getSpecialityByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getTypeFareByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeFareByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		...
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getByIdNegotiationTabColmedica(filters);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getCodeIpsByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getCodeIpsByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getCodeCupsByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getCodeCupsByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getCodeIssByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getCodeIssByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const getTypeReferenceByIdNegotiationTabColmedica: RequestHandler = async (req, res) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeReferenceByIdNegotiationTabColmedica(id_NegotiationTabColmedica);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
		console.error(err);
		res.status(500).json({ message: parseMessageI18n('error_server', req) });
...
export const updateNegotiationTabCupsColmedicaController: RequestHandler = async (req, res) => {
	try {
		...
		const {
			...
			message,
			data: resultData
		} = await repository.updateNegotiationTabCupsColmedicaController(data);
...
export const getTypeFareByIdNegotiationTabColmedicaAmbulatorio: RequestHandler = async (
	req,
	res
) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeFareByIdNegotiationTabColmedicaAmbulatorio(
				id_NegotiationTabColmedica
			);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
...
export const getTypeFareByIdNegotiationTabColmedicaHospitalario: RequestHandler = async (
	req,
	res
) => {
	try {
		const id_NegotiationTabColmedica = req.query.id_NegotiationTabColmedica;
		const { code, message, ...resto }: IResponse<IResponseCreate> =
			await repository.getTypeFareByIdNegotiationTabColmedicaHospitalario(
				id_NegotiationTabColmedica
			);
		res.status(code).json({ message: parseMessageI18n(message, req), ...resto });
	} catch (err) {
